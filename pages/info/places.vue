<template>
    <v-container>
        ...
        <template v-slot:top>
            <v-toolbar flat>
                <v-badge :content="get('count')">Места проведения</v-badge>
                <v-spacer></v-spacer>
...
export default {
    ...
    data(){
        return {
            ...
            s: null,
            headers: [
                { text: 'Наименование', value: 'UF_PLACE' },
                { text: 'Исп.', value: 'NUMS', cellClass: "text-right"},
    ...
    async fetch(){
        try {
            this.all = await this.$store.dispatch("data/list", "places");
            console.log('places', this.all);
        } catch(e){
            this.all = [];
            console.log('ERR (Places)', e);
        }
    },
    computed: {
        places(){
