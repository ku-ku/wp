...
class WpImport  {
    ...
    private function getData(){
        ...
        $stfs = new CHLBTEntity('WpStaffing');
    ...
    public function doPlan(){
        ...
        foreach($this->plan as $p){
            ...
            $iid = $adds->exists();
            if ($iid > 0){
                $args = array( 'select' => array('ID'), 'filter' => array('=ID' => $iid) );
                if ( count($entity->list($args)) > 0 ){
                    continue;
                }
            }
